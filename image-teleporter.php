...
Plugin URI: http://internetmedicineman.github.io/image-teleporter/
...
function bml_it_find_imgs ( $post_id ) {
	...
	if( $a && !in_array( $post->post_author, explode( ',', $a ) ) ) {
		return;
	}
	...
	for($i=0; $i<count($imgs); $i++) {
		if (!$processed || !in_array($imgs[$i], $processed)) {
			...
			if (preg_match ('/(\.php|\.aspx?)$/', $filename) ) {
				$filename .= '.jpg';
			}
	...
	// Post meta
	$bml_it_postmetalist = get_option( 'bml_it_postmetalist' );
	$tmp = explode("\n", $bml_it_postmetalist );
	if ( $bml_it_postmetalist && is_array( $tmp ) ) {
		$post_meta = get_post_meta( $meta_post_id );
		foreach ( $tmp as $key => $value ) {
			$value = trim( $value );
			$found = in_array( $value, array_keys( $post_meta ) ) ? 1 : 0;
			if ( '' != $value && $found ) {
				if ( is_array( $post_meta[$value] ) ) {
					$imgs = bml_it_get_img_links( $post_meta[$value] );
					$new_imgs = array();
					...
					foreach ( $imgs as $img ) {
						...
						// Download and attach image to the post
						$imgid      = bml_it_sideload( $img, $filename, $meta_post_id );
						$new_imgs[] = wp_get_attachment_url( $imgid );

						$bml_it_count++;
					}

					// Relplace
					for ( $i=0; $i< count( $post_meta[$value] ); $i++ ) {
						$replace = str_replace( $imgs, $new_imgs, $post_meta[$value] );
						$replace = is_array( $replace ) ? array_shift( $replace ) : $replace;
						$post_meta[$value][$i] = $replace;
					}

					// Update post mets
					if ( count( $post_meta[$value]) > 1 ) {
						update_post_meta( $meta_post_id, $value, $post_meta[$value] );
					} else {
						update_post_meta( $meta_post_id, $value, array_shift( $post_meta[$value] ) );
					}
				}
			}
		}
...
function bml_it_get_img_links( $post_meta ) {
	$w = get_option('bml_it_whichimgs');
	$s = get_option('siteurl');

	$result = array();

	foreach ( $post_meta as $post_meta_val ) {
		preg_match_all('!http://[^?#]+\.(?:jpe?g|png|gif)!Ui' , $post_meta_val , $matches );
		$matches_found = isset( $matches[0] ) ? count( $matches[0] ) : 0;
		if ( $matches_found > 0 ) {
			for ( $i=0; $i < $matches_found; $i++ ) {
				...
				if ( $s != substr( $uri, 0, strlen( $s ) ) ) {
					//only match Flickr images?
					if( $w == 'All' || ( $w == 'Flickr' && preg_match('/^http:\/\/[a-z0-9]+\.static\.flickr\.com\//', $uri ) ) ) {
						$result[] = $matches[0][$i];
					}
				}
			}
...
function bml_it_backcatalog ($batch, $offset) {
	...
	$cpt = get_option('bml_it_posttypelist');
	$cpt_list = ( $cpt ) ? array_map( 'trim', explode( "\n", $cpt ) ) : array();
	...
	if ( $batch ) {
		if ( $offset ) {
			$pp = new WP_Query( array( 'post_type' => array_merge( array( 'post', 'page' ), $cpt_list ), 'numberposts' => $batch, 'offset' =>  $offset, 'post_status' => 'publish' ) );
		} else {
			$pp = new WP_Query( array( 'post_type' => array_merge( array( 'post', 'page' ), $cpt_list ), 'post_status' => 'publish', 'posts_per_page' => -1 ) );
		}
	} else {
		$pp = new WP_Query( array( 'post_type' => array_merge( array( 'post', 'page' ), $cpt_list ), 'post_status' => 'publish', 'posts_per_page' => -1 ) );
	}
...
function bml_it_menu () {
	add_options_page( 'Image Teleporter', 'Image Teleporter', 'manage_options', 'bml_it', 'bml_it_options' );
}
...
function bml_it_install () {
	...
	$bml_it_postmetalist = get_option('bml_it_postmetalist');
	$bml_it_posttypelist = get_option('bml_it_posttypelist');

	if( !$whichimgs ) {
		update_option( 'bml_it_whichimgs', 'All' );
	}

	if( !$replacesrc ) {
		update_option( 'bml_it_replacesrc', 'replace' );
	}

	if( !$custtagname ) {
		update_option( 'bml_it_custtagname', 'imgteleport' );
	}

	if( !$catlist ) {
		update_option( 'bml_it_catlist', '' );
	}

	if( !$authlist ) {
		update_option( 'bml_it_authlist', '' );
	}

	if( !$bml_it_postmetalist ) {
		update_option( 'bml_it_postmetalist', '' );
	}

	if( !$bml_it_posttypelist ) {
		update_option( 'bml_it_posttypelist', '' );
	}
}
...
function bml_it_options () {
	...
	if ( $action=='update' ) {
		...
		update_option('bml_it_postmetalist', $_POST['bml_it_postmetalist'] );
		update_option('bml_it_posttypelist', $_POST['bml_it_posttypelist'] );

		if( $_POST['bml_it_catlist'] ) {
			update_option('bml_it_catlist', $_cats );
		}
	...
	?>
	<tr align="top">
		<th scope="row"><strong>Apply to these post meta fields:</strong></th>
		<td>
			<textarea name="bml_it_postmetalist" id="bml_it_postmetalist" class="large-text code" rows="10"><?php echo get_option( 'bml_it_postmetalist' ); ?></textarea>
			<p style="font-size: 12px; margin: 8px 0px 0px 0px;">Define each custom post field in newline.</p>
		</td>
	</tr>
	<?php
	...
	?>
	<tr align="top">
		<th scope="row"><strong>Apply to these custom post types:</strong></th>
		<td>
			<textarea name="bml_it_posttypelist" id="bml_it_posttypelist" class="large-text code" rows="10"><?php echo get_option( 'bml_it_posttypelist' ); ?></textarea>
			<p style="font-size: 12px; margin: 8px 0px 0px 0px;">Define each custom post type in newline.</p>
		</td>
	</tr>
	<?php
	...
	// Process Pre-existing Posts/Pages
	echo '<form name="bml_it-backcatalog" method="post" action="">';
	echo '<div class="wrap">';
